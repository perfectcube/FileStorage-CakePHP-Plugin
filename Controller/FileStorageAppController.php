...
class FileStorageAppController extends AppController {
	...
	protected function _detectModelByFileType ($mime_type) {
		...
		$ext = array_search($mime_type, $this->mime_types_map);
		foreach ($this->uses as $use) {
			$model = explode(".", $use);
			$model = isset($model[1]) ? $model[1] : $model[0];
			if(isset($this->$model->actsAs['FileStorage.UploadValidator']['allowedExtensions']) && !empty($this->$model->actsAs['FileStorage.UploadValidator']['allowedExtensions'])) {
				if(array_search($ext, $this->$model->actsAs['FileStorage.UploadValidator']['allowedExtensions']) !== false) {
					return $model;
				}
			}
		}
